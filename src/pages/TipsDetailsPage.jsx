...
const TipsDetailsPage = () => {
  ...
  const { initialValue, handleLikesUpdate } = useContext(likeContext);

  return (
    <div className='max-w-9/12 mx-auto'>
      <div className='text-secondary text-lg flex gap-2 items-center mt-4 cursor-pointer' onClick={() => navigate('/tips')}><span><FaArrowLeft /></span>Back To Tips</div>
      <div className='grid grid-cols-12 gap-6 items-start py-8'>
        ...
        <div className='col-span-4 bg-white shadow-2xl p-4 inline-block'>
          ...
          <div className='grid grid-cols-2 gap-4'>
            <div className='flex items-center gap-2 bg-primary text-white py-2 rounded-full justify-center text-lg cursor-pointer' onClick={handleLikesUpdate}>
              <button className={`cursor-pointer ${initialValue > 0 ? 'text-secondary' : ''}`}><FaHeart /> </button>
              <button className='cursor-pointer'>{initialValue}</button>
            </div>
